...
export default function AdminWeddingDetailPage() {
    ...
    const handleEditGift = (gift: Gift) => {
        navigate(`/collect/${id}?giftId=${gift.id}`);
    };

    const handlePrintGift = async (giftId: number) => {
        try {
            await client.post(`/weddings/${id}/gifts/${giftId}/thermal-print`, { lang });
            toast.success('🖨️ Printed!');
        } catch (err: any) {
            toast.error(err.response?.data?.error || 'Print failed - check printer connection');
        }
    };
        ...
        <div className="page">
            <div className="container">
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', marginBottom: '24px' }}>
                    <Link to="/admin" style={{ color: 'var(--text-muted)', textDecoration: 'none', fontSize: '0.9rem', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
